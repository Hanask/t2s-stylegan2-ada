...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self._xflip[idx]:
            ...
            image = image[:, :, ::-1]
        #Return the respective Embedding
        return image.copy(), self.get_label(idx), self._load_image_emb(idx), self._image_fnames[idx]
...
class ImageFolderDataset(Dataset):
    def __init__(self,
        ...
        **super_kwargs,         # Additional arguments for the Dataset base class.
        ...
        PIL.Image.init()
        self._image_fnames_not_sorted = [fname for fname in self._all_fnames if self._file_ext(fname) in PIL.Image.EXTENSION]
    ...
    def _load_image_emb(self, raw_idx):
        ...
        with self._open_file(fname) as f:
            ...
            try:
                ...
                mean = np.mean(embs,axis=0)
                std = np.std(embs,axis=0)
            new_emb = np.random.normal(mean,std)
        new_emb = torch.from_numpy(new_emb).detach().cpu().float()
        return new_emb
