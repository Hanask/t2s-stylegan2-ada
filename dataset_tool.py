'''
    This script was adapted from Stylegan2, to incorporate embeddings.
'''
...
def open_image_folder(source_dir, *, max_images: Optional[int]):
    ...
    def iterate_images():
        for idx, fname in enumerate(input_images):
            ...
            yield dict(img=img, label=labels.get(arch_fname), image_path=input_images[idx])
            if idx >= max_idx-1:
                break
...
def convert_dataset(
    ...
    Embedding source is a pickle dictionary that contains image path and their captions encoded.
    ...
    num_files, input_iter, all_image_paths = open_dataset(source, max_images=max_images)
    ...
    for idx, image in tqdm(enumerate(input_iter), total=num_files):
        ...
        if not os.path.exists(dest.replace(".zip","")+"/"+archive_fname.split("/")[0]):
            os.makedirs(dest.replace(".zip","")+"/"+archive_fname.split("/")[0])
        pickle.dump(emb,open(os.path.join(archive_root_dir, dest.replace(".zip","")+"/"+archive_fname.replace(".png",".pickle")),"wb"))
    metadata = {
